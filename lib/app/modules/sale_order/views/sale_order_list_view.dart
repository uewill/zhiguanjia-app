...
import 'sale_order_create_view.dart';

class SaleOrderListView extends StatelessWidget {
  const SaleOrderListView({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: TDNavBar(
        ...
        rightBarItems: [
          TDNavBarItem(icon: TDIcons.add, iconColor: Colors.white, action: () => Get.to(() => const SaleOrderCreateView())),
        ],
      ),
      body: Obx(() {
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const TDText('暂无销售订单', style: TextStyle(color: Colors.grey)),
                const SizedBox(height: 16),
                TDButton(
                  text: '新建销售订单',
                  theme: TDButtonTheme.primary,
                  onTap: () => Get.to(() => const SaleOrderCreateView()),
                ),
              ],
            ),
          );
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor: const Color(0xFF2FC27D),
        onPressed: () => Get.to(() => const SaleOrderCreateView()),
        child: const Icon(Icons.add, color: Colors.white),
      ),
    );
  }

  Widget _buildOrderCard(SaleOrderController controller, dynamic order) {
    Color statusColor;
    String statusText;
    IconData statusIcon;
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 4),
                    TDText(order.customerName, style: const TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 4),
                    TDText('出库仓库: ${order.warehouseName}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                  ],
                ),
  ...
  void _showConvertDialog(SaleOrderController controller, int orderId) {
    Get.dialog(
      AlertDialog(
        title: const Text('转销售出库'),
        content: const Text('确认后将根据此订单生成销售出库单，确认吗？'),
        actions: [
          TextButton(onPressed: () => Get.back(), child: const Text('取消')),
          TextButton(
            onPressed: () async {
              Get.back();
              await controller.convertToSale(orderId);
            },
  ...
  void _showCancelDialog(SaleOrderController controller, int orderId) {
    Get.dialog(
      AlertDialog(
        title: const Text('取消订单'),
        content: const Text('确定要取消此销售订单吗？'),
        actions: [
